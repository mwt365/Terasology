// Copyright 2026 The Terasology Foundation
// SPDX-License-Identifier: Apache-2.0
package org.terasology.module.advancednpcai.actions.movement;

import org.joml.Vector3f;
import org.terasology.engine.logic.behavior.BehaviorAction;
import org.terasology.engine.logic.behavior.core.Actor;
import org.terasology.engine.logic.behavior.core.BaseAction;
import org.terasology.engine.logic.behavior.core.BehaviorState;
import org.terasology.engine.logic.location.LocationComponent;
import org.terasology.engine.utilities.random.FastRandom;
import org.terasology.engine.utilities.random.Random;

@BehaviorAction(name = "wander")
public class WanderAction extends BaseAction {
    private float wanderRadius = 8f;
    private float wanderSpeed = 1.0f;

    @Override
    public void construct(Actor actor) {
        LocationComponent loc = actor.getComponent(LocationComponent.class);
        if (loc != null) {
            Random random = new FastRandom();
            float angle = random.nextFloat() * 2f * (float) Math.PI;
            float dist = random.nextFloat() * wanderRadius;
            Vector3f currentPos = new Vector3f(loc.getLocalPosition());
            Vector3f wanderTarget = new Vector3f(
                    currentPos.x + (float) Math.cos(angle) * dist,
                    currentPos.y,
                    currentPos.z + (float) Math.sin(angle) * dist);
            actor.setValue(getId(), wanderTarget);
        }
    }

    @Override
    public BehaviorState modify(Actor actor, BehaviorState result) {
        Vector3f wanderTarget = actor.getValue(getId());
        if (wanderTarget == null) {
            return BehaviorState.FAILURE;
        }

        LocationComponent loc = actor.getComponent(LocationComponent.class);
        if (loc == null) {
            return BehaviorState.FAILURE;
        }

        Vector3f currentPos = new Vector3f(loc.getLocalPosition());
        float distance = currentPos.distance(wanderTarget);
        if (distance <= 1.0f) {
            return BehaviorState.SUCCESS;
        }

        Vector3f direction = new Vector3f(wanderTarget).sub(currentPos).normalize();
        float step = wanderSpeed * actor.getDelta();
        if (step > distance) {
            step = distance;
        }

        Vector3f newPos = new Vector3f(currentPos).add(
                direction.x * step, 0, direction.z * step);
        loc.position.set(newPos);
        actor.save(loc);

        return BehaviorState.RUNNING;
    }
}
